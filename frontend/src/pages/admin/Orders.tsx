...
const AdminOrders: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        setError('Não foi possível carregar as ordens de serviço. Por favor, tente novamente mais tarde.');
        setLoading(false);
      }
    };
  ...
  // Filtrar ordens com base no status selecionado e termo de busca
      ...
      <div className="mb-6 flex flex-col md:flex-row gap-4">
        ...
        <div className="md:ml-auto">
          <input
            ...
            className="px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
            value={searchTerm}
                  ...
                  <tr key={order.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-3">
                        <Link to={`/admin/ordens/${order.id}`} className="text-[var(--primary)] hover:text-[var(--primary-dark)]">
                          Ver
                        </Link>
                        <Link to={`/admin/ordens/${order.id}/editar`} className="text-[var(--primary)] hover:text-[var(--primary-dark)]">
                          Editar
                        </Link>
                        <button 
                          onClick={() => {
                            if (window.confirm('Tem certeza que deseja excluir esta ordem de serviço?')) {
                              ...
                              serviceOrderService.deleteOrder(order.id)
                                .then(() => {
